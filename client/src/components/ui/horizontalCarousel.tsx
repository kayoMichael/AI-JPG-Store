...
export const HorizontalCarousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const checkScrollability = () => {
    if (carouselRef.current) {
      ...
      setCanScrollRight(scrollLeft < scrollWidth - clientWidth - 10); // Added small threshold
    }
  };
  ...
  return (
    <CarouselContext.Provider value={{ onCardClose: handleCardClose, currentIndex }}>
      <div className="relative w-full">
        <div
          className="flex w-full overflow-x-scroll scroll-smooth [scrollbar-width:none] pb-10"
          ref={carouselRef}
          ...
          <div className="flex flex-row gap-4 pl-4">
            {items.map((item, index) => (
              <motion.div
                ...
                key={'card' + index}
                className={cn('rounded-3xl', index === items.length - 1 ? 'pr-4' : '')}
              >
                {item}
              </motion.div>
            ))}
